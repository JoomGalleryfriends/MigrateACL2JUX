...
/******************************************************************************\
**   Migration of DB and Files from JoomGallery 2.1 to Joomgallery 3 JUX      **
 ...
 * @since   3.0
 */
class JoomMigrateAcl2Jux extends JoomMigration
{
  /**
  ...
  public function check($dirs = array(), $tables = array(), $xml = false, $min_version = false, $max_version = false)
  {
    ...
    {
      JFactory::getLanguage()->load('com_joomgallery.migrateacl2jux');
      $this->_mainframe->redirect('index.php?option='._JOOM_OPTION.'&controller=migration', JText::_('FILES_JOOMGALLERY_MIGRATION_ACL2JUX_WRONG_PATH2JOOMLA'), 'notice');
    }

    if(!$this->otherDatabase && $this->prefix == $this->_db->getPrefix())
    {
      JFactory::getLanguage()->load('com_joomgallery.migrateacl2jux');
      $this->_mainframe->redirect('index.php?option='._JOOM_OPTION.'&controller=migration', JText::_('FILES_JOOMGALLERY_MIGRATION_ACL2JUX_WRONG_PREFIX'), 'notice');
    }
    ...
    $tables       = array($this->table_images,
                          ...
                          $this->table_users,
                          $this->table_votes,
                          $this->table_img_details,
                          $this->table_cat_details);
    $xml = null;
    if($this->path != '-')
    {
  ...
  protected function doMigration()
  {
    ...
    switch($task)
    {
      ...
      case 'config':
        ...
        // Break intentionally omitted
      case 'imagedetails':
        $this->migrateImageDetails();
        // Break intentionally omitted
      case 'categorydetails':
        $this->migrateCategoryDetails();
        // Break intentionally omitted
      default:
        break;
    }
  ...
  /**
   * Returns the maximum category ID of JoomGallery 2.1
   *
   * @return  int The maximum category ID of JoomGallery 2.1
   * @since   3.0
   */
  ...
  protected function migrateCategories()
  {
    ...
    $this->prepareTable($query, $this->table_categories, 'parent_id', array(0));

    while($cat = $this->getNextObject())
    {
      if($cat->cid == 1)
      {
        $this->markAsMigrated(1, 'cid', $this->table_categories);
  ...
  protected function migrateConfig()
  {
    ...
    // 'jg_bigpic_open' disabled, not all values can be migrated
    $migrateable_settings = array('jg_use_real_paths', 'jg_checkupdate',
                                  ...
                                  'jg_showrating', 'jg_maxvoting', 'jg_ratingcalctype', 'jg_ratingdisplaytype', 'jg_ajaxrating',
                                  ...
                                  'jg_showgalleryhead', 'jg_showpathway', 'jg_completebreadcrumbs', 'jg_showallpics', 'jg_showallhits', 'jg_showbacklink', 'jg_suppresscredits', 'jg_showrestrictedcats', 'jg_showrestrictedhint',
                                  'jg_showallpicstoadmin', 'jg_showminithumbs', 'jg_openjs_padding', 'jg_openjs_background', 'jg_dhtml_border', 'jg_show_title_in_popup', 'jg_show_description_in_popup', 'jg_lightbox_speed', 'jg_lightbox_slide_all', 'jg_resize_js_image', 'jg_disable_rightclick_original',
                                  ...
                                  'jg_showexifdata', 'jg_showgeotagging', 'jg_geotaggingkey', 'jg_subifdtags', 'jg_ifdotags', 'jg_gpstags',
   ...
   * @since   3.0
   */
  protected function migrateImageDetails()
  {
    $selectQuery = $this->_db2->getQuery(true)
                ->select('a.*')
                ->from($this->table_img_details.' AS a')
                ->innerJoin($this->table_images.' AS i ON a.id = i.id');

    if(!$this->otherDatabase)
    {
      if(!$this->checkTime())
      {
        $this->refresh('imagedetails');
      }

      $query = 'INSERT INTO '._JOOM_TABLE_IMAGE_DETAILS.' '.$selectQuery;
      $this->_db->setQuery($query);
      if($this->runQuery())
      {
        $this->writeLogfile('image details successfully migrated');
      }
      else
      {
        $this->writeLogfile('Error migrating the image details');
      }
    ...
    $query = $this->_db->getQuery(true)
          ...
          ->columns('id, details_key, details_value, ordering');

    while($row = $this->getNextObject())
    {
      $query->clear('values')
            ->values($query->q($row->id).','.$query->q($row->details_key).','.$query->q($row->details_value).','.$query->q($row->ordering));
      $this->_db->setQuery($query);
      $this->runQuery();
   ...
   * @since   3.0
   */
  protected function migrateCategoryDetails()
  {
    $selectQuery = $this->_db2->getQuery(true)
                ->select('a.*')
                ->from($this->table_cat_details.' AS a')
                ->innerJoin($this->table_categories.' AS c ON a.id = c.cid');

    if(!$this->otherDatabase)
    {
      if(!$this->checkTime())
      {
        $this->refresh('categorydetails');
      }

      $query = 'INSERT INTO '._JOOM_TABLE_CATEGORY_DETAILS.' '.$selectQuery;
      $this->_db->setQuery($query);
      if($this->runQuery())
      {
        $this->writeLogfile('category details successfully migrated');
      }
      else
      {
        $this->writeLogfile('Error migrating the category details');
      }
